creator_setting = {
    ...
    When creating the new_method_executable_RESULT tag and new_method_code_RESULT, describe the method created so far and the results of its execution, so that the method will not become non-executable and will better meet the objectives.
    ...
    Attention.
        ...
        - When creating new_method_code, one must be careful not to make the same mistake, since the new_method_executable_RESULT and new_method_code_RESULT tags contain the results of the previous creation.In addition, the method should be made to follow the objectives more closely by devising a method at the time of creation.
