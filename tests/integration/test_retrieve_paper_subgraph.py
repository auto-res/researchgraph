...
from airas.retrieve_paper_subgraph.retrieve_paper_subgraph import RetrievePaperSubgraph
...
@patch(
    "airas.retrieve_paper_subgraph.nodes.web_scrape_node.web_scrape_node",
    return_value=["dummy scraped result"],
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.extract_paper_title_node.extract_paper_title_node",
    return_value=["Test Paper"],
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.arxiv_api_node.ArxivNode.execute",
    return_value=[
        {
            "arxiv_id": "1234",
...
@patch(
    "airas.retrieve_paper_subgraph.nodes.retrieve_arxiv_text_node.RetrievearXivTextNode.execute",
    return_value="full text",
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.extract_github_url_node.ExtractGithubUrlNode.execute",
    return_value="https://github.com/test/repo",
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.summarize_paper_node.summarize_paper_node",
    return_value=("main", "method", "exp", "lim", "future"),
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.select_best_paper_node.select_best_paper_node",
    return_value=["1234"],
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.generate_queries_node.generate_queries_node",
