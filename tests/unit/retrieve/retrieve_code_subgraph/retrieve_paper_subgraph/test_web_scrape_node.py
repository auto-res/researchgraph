...
@patch(
    "airas.retrieve_paper_subgraph.nodes.web_scrape_node.FIRE_CRAWL_API_KEY",
    "dummy-key",
)
@patch(
    "airas.retrieve_paper_subgraph.nodes.web_scrape_node.firecrawl_scrape",
    return_value={"data": {"markdown": "scraped content"}},
)
def test_web_scrape_node_success(mock_scrape, mock_key):
    queries = ["query1"]
...
@patch("airas.retrieve_paper_subgraph.nodes.web_scrape_node.FIRE_CRAWL_API_KEY", None)
def test_web_scrape_node_no_api_key(mock_key):
    queries = ["query1"]
    scrape_urls = ["https://example.com"]
    result = web_scrape_node(queries, scrape_urls)
    assert result == []
