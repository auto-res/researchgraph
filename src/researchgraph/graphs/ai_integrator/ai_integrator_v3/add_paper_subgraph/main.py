...
class AddPaperSubgraph(PaperSubgraph):
    def __init__(self, *args, **kwargs):
        ...
        self.graph_builder.add_node("generate_queries_node", self._generate_queries_node) 
    ...
    def __call__(self, state: PaperState) -> dict:
        result = self.graph.invoke(state, debug=True, config={"recursion_limit": 100})
        self._cleanup_result(result)
        result = {
            f"add_{k}" if k != "base_selected_paper" else k: v
