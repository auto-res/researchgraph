...
from researchgraph.retrieve_paper_subgraph.nodes.extract_paper_title_node import extract_paper_title_node
from researchgraph.retrieve_paper_subgraph.nodes.arxiv_api_node import ArxivNode
...
class RetrievePaperSubgraph:
    ...
    def _extract_paper_title_node(self, state: RetrievePaperState) -> dict:
        ...
        return {
            "extracted_paper_titles": extracted_paper_titles
        }
    ...
    def _search_arxiv_node(self, state: RetrievePaperState) -> dict:
        ...
        batch_paper_titles = extract_paper_titles[:min(len(extract_paper_titles), self.arxiv_query_batch_size)]
    ...
    def _prepare_state(self, state: RetrievePaperState) -> dict:
        ...
        add_github_urls = [paper_info.github_url for paper_info in state["selected_add_paper_info_list"]]
        add_method_texts = [paper_info.model_dump_json() for paper_info in state["selected_add_paper_info_list"]]
    ...
    def build_graph(self) -> CompiledGraph:
        ...
        graph_builder.add_node("base_extract_paper_title_node", self._extract_paper_title_node)
        graph_builder.add_node("base_search_arxiv_node", self._search_arxiv_node) # TODO: 検索結果が空ならEND
        graph_builder.add_node("base_retrieve_arxiv_full_text_node", self._retrieve_arxiv_full_text_node)
        graph_builder.add_node("base_extract_github_urls_node", self._extract_github_url_node)
        graph_builder.add_node("base_summarize_paper_node", self._summarize_base_paper_node)
        graph_builder.add_node("base_select_best_paper_node", self._base_select_best_paper_node)
        ...
        graph_builder.add_node("add_extract_paper_title_node", self._extract_paper_title_node)
        graph_builder.add_node("add_search_arxiv_node", self._search_arxiv_node) # TODO: 検索結果が空ならEND
        graph_builder.add_node("add_retrieve_arxiv_full_text_node", self._retrieve_arxiv_full_text_node)
        graph_builder.add_node("add_extract_github_urls_node", self._extract_github_url_node)
        graph_builder.add_node("add_summarize_paper_node", self._summarize_add_paper_node)
        graph_builder.add_node("add_select_best_paper_node", self._add_select_best_paper_node)
