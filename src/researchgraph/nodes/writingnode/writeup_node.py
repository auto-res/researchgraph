...
class WriteupNode(Node):
    def __init__(
        ...
        self.target_sections = target_sections or [
             "Title", "Abstract", "Introduction", "Related work", "Background", "Method", "Experimental setup", "Results", "Conclusions"
        ]
    ...
    def _clean_meta_information(self, text: str) -> str:
        ...
        text = re.sub(combined_pattern, "", text, flags=re.DOTALL | re.IGNORECASE | re.MULTILINE)

        text = re.sub(r"^#{1,}\s*", "", text, flags=re.MULTILINE)   # Remove '###' but keep the title content
        text = re.sub(r"([_%&#$])", r"\\\1", text)                  # Escape LaTeX special characters: _, %, &, #, $
        text = re.sub(r"\*\*(.*?)\*\*", r"\\textbf{\1}", text)      # Convert Markdown bold (**text**) to LaTeX bold (\textbf{text})
        text = re.sub(r"(\n\s*){2,}", "\n\n", text.strip())         # Replace consecutive blank lines (2 or more) with a single blank line
