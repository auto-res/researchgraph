...
def test_request_github_actions_workflow_execution(mock_retry_request, mock_fetch_api_data, execute_github_actions_workflow_node, test_environment):
    ...
    response = execute_github_actions_workflow_node._request_github_actions_workflow_execution(
...
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.fetch_api_data")
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.retry_request")
def test_request_github_actions_workflow_info_before_execution(mock_retry_request, mock_fetch_api_data, execute_github_actions_workflow_node, test_environment):
...
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.fetch_api_data")
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.retry_request")
def test_request_github_actions_workflow_info_after_execution(mock_retry_request, mock_fetch_api_data, execute_github_actions_workflow_node, test_environment):
...
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.fetch_api_data")
@patch("researchgraph.executor_subgraph.nodes.execute_github_actions_workflow.retry_request")
def test_execute(mock_retry_request, mock_fetch_api_data, execute_github_actions_workflow_node, test_environment):
    """ 正常系テスト: ExecuteGithubActionsWorkflowNode の execute メソッドが正しく動作するか """
    mock_fetch_api_data.return_value = {
        "workflow_runs": [{"id": 1, "status": "completed", "created_at": "2024-02-12T12:34:56Z"}]
    }
    mock_retry_request.side_effect = lambda func, *args, **kwargs: func(*args, **kwargs)

    with patch.object(execute_github_actions_workflow_node, "_request_github_actions_workflow_execution") as mock_execute_workflow:
        mock_execute_workflow.return_value = {"status": "success"}

        with patch.object(execute_github_actions_workflow_node, "_request_github_actions_workflow_info_after_execution") as mock_info_after_execution:
            mock_info_after_execution.return_value = {
                "workflow_runs": [
                    {"id": 1, "status": "completed", "created_at": "2024-02-12T12:34:56Z"},
                    {"id": 2, "status": "completed", "created_at": "2024-02-12T13:00:00Z"}
                ]
            }
