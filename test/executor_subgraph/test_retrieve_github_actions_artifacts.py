...
def test_request_github_actions_artifacts(mock_retry_request, mock_fetch_api_data, retrieve_github_actions_artifacts_node, test_environment):
...
def test_parse_artifacts_info(retrieve_github_actions_artifacts_node, test_environment):
    ...
    artifacts_info = {
        "artifacts": [
            {"name": "artifact1", "archive_download_url": "https://github.com/test/artifact1.zip", "workflow_run": {"id": 123456789}},
            {"name": "artifact2", "archive_download_url": "https://github.com/test/artifact2.zip", "workflow_run": {"id": 987654321}},
        ]
    }

    parsed_info = retrieve_github_actions_artifacts_node._parse_artifacts_info(artifacts_info, test_environment["workflow_run_id"])
...
@patch("researchgraph.executor_subgraph.nodes.retrieve_github_actions_artifacts.fetch_api_data")
@patch("researchgraph.executor_subgraph.nodes.retrieve_github_actions_artifacts.retry_request")
def test_request_download_artifacts(mock_retry_request, mock_fetch_api_data, retrieve_github_actions_artifacts_node, test_environment):
    ...
    with patch.object(retrieve_github_actions_artifacts_node, "_zip_to_txt") as mock_zip_to_txt:
...
def test_zip_to_txt(mock_open_file, mock_zipfile, mock_exists, mock_remove, retrieve_github_actions_artifacts_node, test_environment):
    ...
    retrieve_github_actions_artifacts_node._zip_to_txt(response_mock, iteration_save_dir, key)
...
@patch("os.makedirs")
def test_execute(mock_makedirs, mock_open_file, mock_retry_request, mock_fetch_api_data, retrieve_github_actions_artifacts_node, test_environment):
    ...
    with patch.object(retrieve_github_actions_artifacts_node, "_request_download_artifacts") as mock_download:
        mock_download.return_value = None

        output_text_data, error_text_data = retrieve_github_actions_artifacts_node.execute(
        ...
        mock_open_file.assert_any_call(os.path.join(test_environment["save_dir"] + "/iteration_1", "output.txt"), "r")
        mock_open_file.assert_any_call(os.path.join(test_environment["save_dir"] + "/iteration_1", "error.txt"), "r")
